...
A simulated test set (with 50 splitting reads and 13 spanning reads for each fusion) is in the simulation-test-data folder. (2 fastq file and 1 simulated fusion list) Also, parameter_file.txt for this dataset is provided.
...
Reference Dataset
1.	Genome.fa and build the bowtie index. For bowtie website. [Note: the fa file must use ¡°chr*¡± as chromosome rather than use just 1-22, X, Y and M.] The hg19.fa I used can be also downloaded in the reference folder. (You need to build bowtie index yourself based on the fa by using bowtie or bowtie2.)
